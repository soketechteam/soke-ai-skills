...
// 创建实例
const skillInstance = new AiExamSkill();
...
export async function execute(params) {
  ...
  if (!config.appKey) {
    ...
    if (!appKey || !appSecret || !corpId) {
      throw new Error('缺少必要的环境变量: APP_KEY, APP_SECRET, CORP_ID');
    }

    await skillInstance.initialize(appKey, appSecret, corpId);
  }

  // 根据 action 执行不同操作
  switch (action) {
    case 'createAndAssignExam':
      return await skillInstance.createAndAssignExam(options);

    case 'queryExamResult':
      return await skillInstance.getExamResult(options.examId, options.userName);

    case 'queryBatchExamResults':
      return await skillInstance.batchGetExamResults(options.examId, options.userNames);

    case 'quickCreateExam':
      return await skillInstance.quickCreateExam(
        options.filePath,
        options.examTitle,
        options.creator
      );
