...
export async function execute(params) {
  try {
    if (!CONFIG.appKey || !CONFIG.appSecret || !CONFIG.corpId) {
      throw new Error('缺少必需的环境变量: APP_KEY, APP_SECRET, CORP_ID');
    }

    const { userName, deptName, isNewEmployee } = params;
    const isNew = isNewEmployee ? '1' : undefined;
    ...
    if (userName) {
      ...
      try {
        ...
        if (profiles.length === 0) {
          return {
            error: `找到用户 "${userName}"，但该用户暂无学习档案数据。`
          };
        }

        return formatLearningProfiles(profiles);
      } catch (error) {
        if (error.message.includes('51003')) {
          return {
            error: `找到用户 "${userName}"，但该用户暂无学习档案数据。`
          };
        }
